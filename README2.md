...
TDD란? : Test-Driven Development는 테스트가 개발의 중심이 되는 소프트웨어 개발 방법론

테스트 코드를 작성한 뒤에 코드개발을 진행하는 방식
장점: 코드안정성, 재설계시간의 단축, 디버깅 시간의 단축 등
단점: 테스트 코드까지 만들어야하니 개발시간이 오래걸린다. 즉, 간단한 개발이나 단발성 개발인 경우 시간낭비

앱생성까지만 하고 소스코드 작성하기 전에, test.py 만들고 테스트 요구사항에 맞춘 코드를 만든 후, Fail->Success로 만들어나가는 과정으로 진행
success가 뜰때마다 커밋하면서 돌아갈 수 있는 지점을 만듬


## 231117

### Django JWT
https://paullabworkspace.notion.site/Django-JWT-3ed748c99d184c75b4f901e382b69527
JWT = JSON Web Tokens

왜 쓰는가? : 사용자 인증에 매우 유용, 정보교환이 효율적, 세션관리

토큰에 유저 정보가 담겨있기 때문에 다른 DB를 사용하지 않아도 되는데, 그 정보를 담지 않는다면 JWT를 사용하는데 유리한 방법이 아니다
JWT는 만료 시간이 있기 때문에 어느정도 보안성이 보장된다

구성은?: 헤더, 페이로드, 서명(비밀키) BASE64

Refresh Token은 Access Token보다 조금 길게 해주는 편

JWT 인증방식으로 회원가입, 로그인 구현

Postman사이트,Thunder Client 확장기능 으로 테스트 해보기
...
오종필 강사님 - (주)엘케이플러스컨설팅 대표컨설턴트, 삼성전자 인사팀, 인사채용 및 인력운영 담당

얼마나 꾸준히 했는가? : 순차적이 아닌, '병렬식'으로 미리미리 여러가지를 조금씩이라도 꾸준히 하자.

구체적인 목표와 계획 : '목표' 와 '계획'은 다르다. ex) 1월에 뭐하기 2월에 뭐하기...

자신감있고 적극적인 마인드 : 생각보다 적극적인 사람이 많이 드물다

전문성에 대한 글은 많이 세세하게 : 모르는 사람이 3,4시간 정도 조사하고 대충 쓴 느낌은 확 티가 난다.

## 231205

### 취업특강2

AI 면접은 '처음 자기소개' 에서 부터 얼마나 어조? 등이 변화했는지를 체크한다.

기본적으론 '회사의 입장' 에서 생각해서 답한다.
...
처음에 생각했던 erd의 이름짓기에 변경이 필요함 (body->content)
